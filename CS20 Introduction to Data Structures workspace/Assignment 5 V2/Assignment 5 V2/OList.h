...
public:
    ...
    OList operator+=(const OList &other) {
        ...
        if (thisTemp == NULL) // If this list is empty
            return *this = other;
        if (&otherTemp != NULL && otherTemp->data < thisTemp->data) {
        ...
        while (i++ < length) {
            while (thisTemp->next != NULL && otherTemp->data > thisTemp->next->data) {
    ...
    void remove(T value, bool recurrence = true) {
        ...
        for (int i = 0; i < size; i++) { // Don't check the rest of the list
            ...
            else if (temp->next != NULL && temp->next->data == value ) { // If in body
                temp1 = temp->next;
                if (temp1 == tail)
                    tail = temp;
