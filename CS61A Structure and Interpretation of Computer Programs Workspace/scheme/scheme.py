...
#    print(target.second)
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        expr = scheme_eval(expressions.second.first, env)
        env.define(target, expr)
        return target
        # END PROBLEM 6
        ...
        env.define(target.first, do_lambda_form(lambda_form, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    elif scheme_falsep(evaluated_expression):
        return evaluated_expression
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    evaluated_expression = scheme_eval(expressions.first, env)
    if scheme_truep(evaluated_expression):
        return evaluated_expression
    else:
