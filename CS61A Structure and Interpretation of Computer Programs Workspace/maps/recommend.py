...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    cluster = [[find_closest(restaurant_location(res), centroids), res]
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    longitude = mean([restaurant_location(elem)[0] for elem in cluster])
    latitude = mean([restaurant_location(elem)[1] for elem in cluster])
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(elem) for elem in cluster]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predicted = [find_predictor(user, reviewed, features) for features in feature_fns]
    return max(predicted, key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for res in restaurants:
        ...
        if res in reviewed:
            d[res_name] = user_rating(user, restaurant_name(res))
        else:
            d[res_name] = predictor(res)
    return d
    # END Question 9
